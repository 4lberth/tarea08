...
export const isAdmin = async (req, res, next) => {
  try {
    ...
    // Si el usuario no tiene el rol de 'admin', respondemos con error 403
...
export const isModerator = async (req, res, next) => {
  try {
    ...
    // Si el usuario no tiene el rol de 'moderator', respondemos con error 403
...
export const isModeratorOrAdmin = async (req, res, next) => {
  try {
    ...
    // Si el usuario tiene al menos uno de los roles, pasamos al siguiente middleware
    if (hasRole) {
      next();
      return;
    }

    // Si no tiene ninguno de los roles requeridos, respondemos con error 403
    res.status(403).json({ message: "¡Se requiere el rol de moderador o administrador!" });
  } catch (error) {
    // Si ocurre un error, respondemos con error 500
    res.status(500).json({ message: error.message });
  }
};
